...
def get_image_info(image_url, artists):
    json_prompt = f"Analyze the image and output information as a valid json object (and only the JSON) with the following fields: 1. description - required. containins a detailed description of what is in the image including prominent colors and vibes and emotions provoked 2. playlistTitle - required. a playlist title that goes with the image description. Example: \"Floating on air: A night with The Boss\" 3. music - optional.  If the image contains information on a band, music artist, or song, output the band name, artist name, or song name in this field. If there isn't anything, don't include this field. 4. genre - required. Output a broad music genre that best encapsulates the vibe of the image. It should be something like pop, country, hip-hop, RNB, etc. 5. subgenre - required.  Choose a more specific and creative subgenre of the genre above that best represents the image. For example, indie pop is not specific enough. Feel free to include decades! 6. mood - required. the general mood of the playlist ex. upbeat, sad, thought-provoking, cozy, etc. 7. songlist - required.  Create a soundtrack of 30 songs for the image that fit the subgenre mentioned above. All of the songs should fit the mood as well, so that the songs do not clash. These should be real songs from real artists that I can find on Spotify. If there was a band, music artist, or song in the image, include only three song from that music. Also, if applicable, include songs from my top artists: {artists}. If any of my top artists have songs fit the subgenre or image description, include them. Try to find niche songs from these artists that fit the subgenre and image description better than their most popular songs. For example (only an example), instead of Cardigan by Taylor Swift, you might choose The Lakes or Seven if they fit the image description better. Try to use as many top artists as possible, however, the main priority is that the songs FIT THE SUBGENRE. Do NOT include songs from these artists if they do not fit the subgenre. These should all be real songs from the artists. The soundtrack should be a third mainstream songs and two thirds underground artists for diversity. All of the songs should flow together and have a similar tone. The songs should include three string fields - title, artist, and reason."
